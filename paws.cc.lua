...
do
	...
	for i = 1, 10 do
	    print("Emper Divine Hub Mobile Loaded!")
	end;
	...
	do
		...
		function Library:ChangeAccent(Color)
			...
			for obj, theme in next, Library.ThemeObjects do
                ...
                if theme:IsA("ImageLabel") and obj == "ImageColor3" then  -- Kiểm tra property
	...
	do
		...
		function Library:New(Properties)
			...
			if UserInputService.TouchEnabled then
			    ...
			    button.Text = '<font color="#d0e300">Star</font><font color="#FFFFFF">Hook</font>'			
			...
			LogoText.FontFace = Font.new("rbxasset://fonts/families/Silkscreen.json")
			LogoText.Text = "Paws.cc"
			LogoText.TextColor3 = Color3.fromRGB(255, 182, 193) 
			...
			LogoText.Size = UDim2.fromOffset(60, 90)
			LogoText.Parent = Holder
			LogoText.ZIndex = 53
            LogoText.Position = UDim2.fromOffset(10, -20)
			...
			Logo.Name = "Logo"
			Logo.Image = "http://www.roblox.com/asset/?id=11351620343"
			Logo.ScaleType = Enum.ScaleType.Fit
			Logo.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			Logo.BackgroundTransparency = 1
			Logo.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Logo.BorderSizePixel = 0
			Logo.Position = UDim2.fromOffset(75, -5)
            ...
            Glow.ZIndex = 0  -- Nằm dưới tất cả nội dung
            Glow.Image = "rbxassetid://5028857084"
            Glow.ImageColor3 = Library.Accent  -- Màu accent
            Glow.ScaleType = Enum.ScaleType.Slice
            Glow.SliceCenter = Rect.new(24, 24, 276, 276)
            Glow.Parent = Holder  -- Làm con của Holder
			--
    ...
    function Library:Watermark(Properties)
        local Watermark = {
            Name = (Properties.Name or Properties.name or "placeholder"),
            AnimateText = nil,
        }
        --
        local Outline = Instance.new("Frame")
        ...
        -- Loop cập nhật MS và văn bản watermark (mỗi 0.5 giây để tránh lag)
        task.spawn(function()
            while task.wait(0.5) do
                ms = math.round(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())
                -- Cập nhật văn bản với FPS và MS (kết hợp với AnimateText nếu có)
                Title.Text = string.format("%s | FPS: %d | MS: %d", Watermark.AnimateText or "Emper Divine", fps, ms)
            end
        end)

        task.spawn(function()
            while task.wait() do
                for i = 1, #"Emper Divine" do
                    Watermark.AnimateText = string.sub("Paws.cc", 1, i) .. ""
                    Title.Text = string.format("%s | FPS: %d | MS: %d", Watermark.AnimateText, fps, ms)
                    task.wait(0.4)
                end

                for i = #"Emper Divine" - 1, 1, -1 do
                    Watermark.AnimateText = string.sub("Paws.cc", 1, i) .. ""
                    Title.Text = string.format("%s | FPS: %d | MS: %d", Watermark.AnimateText, fps, ms)
                    task.wait(0.4)
                end
...
--// window locals
local default_color = Color3.fromRGB(255, 158, 207);
...
local possibleRemotes = {
    ...
    "Bullets",
    "MainRemote",
    "MainRemotes",      -- folder → MainRemoteEvent
    "Packages",         -- Knit path → ToolService.RE.UpdateAim
    "Events",           -- một số game mới dùng Events.RemoteEvent
    "Remotes",          -- vài game dùng Remotes.Main
}


local function getMainRemote()
    if localMainRemote then 
        return localMainRemote 
    end

    local RS = game:GetService("ReplicatedStorage")

    -- 1. Tìm remote trực tiếp trong danh sách
    for _, name in ipairs(possibleRemotes) do
        local remote = RS:FindFirstChild(name)
        if remote and (remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction")) then
            localMainRemote = remote
            return remote
        end
    end

    -- 2. Check MainRemotes.MainRemoteEvent (rất phổ biến)
    local mainRemotes = RS:FindFirstChild("MainRemotes")
    if mainRemotes and mainRemotes:FindFirstChild("MainRemoteEvent") and mainRemotes.MainRemoteEvent:IsA("RemoteEvent") then
        localMainRemote = mainRemotes.MainRemoteEvent
        return localMainRemote
    end

    -- 3. Check Packages → Knit → ToolService.RE.UpdateAim (Arsenal, Blade Ball, v.v…)
        ...
        if knit then
            ...
            if services then
                ...
                if toolService then
                    ...
                    if re and re:FindFirstChild("UpdateAim") then
                        localMainRemote = re.UpdateAim
                        return localMainRemote
                    end
                end
            end
    ...
    -- 4. Check Events folder (một số game mới)
    local events = RS:FindFirstChild("Events")
    if events then
        for _, v in ipairs(events:GetChildren()) do
            if v:IsA("RemoteEvent") and (v.Name:lower():find("main") or v.Name:lower():find("remote") or v.Name:lower():find("aim")) then
                localMainRemote = v
                return v
            end
        end
...
-- Cách dùng (chỉ gọi 1 lần duy nhất, cache lại cho mượt)
local remote = getMainRemote()

if MainRemote then
    print("[REMOTE] Tìm thấy Main Remote:", MainRemote:GetFullName())
    -- dùng MainRemote:FireServer(...)
else
    warn("[REMOTE] Không tìm thấy Main Remote nào → một số chức năng (silent aim, bullet tracers, v.v…) sẽ không hoạt động")
end
--// world
local world = {
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			...
			Parent = screen_gui_2,
			Text = '<font color="rgb(207, 227, 0)">Vhuy</font><font color="rgb(255, 255, 255)">.Ghoul</font>',
    ...
    do
            ...
            utility.new_connection(run_service.Heartbeat, function()
                ...
                if ((target_aim_enabled and target_aim_teleport_enabled and target_aim_teleport_keybind_active) and (is_targetting and target and utility.has_character(target)) and (not target_aim_teleport_destroy_cheaters_bypass or target_aim_teleport_destroy_cheaters_bypass and target.Character.HumanoidRootPart.CFrame.Position.Y >= -10000)) then
					...
					if rootPart then
				    	...
				    	desync_setback.CFrame = old_cframe * CFrame.new(0, rootPart.Size.Y / 2 + 0.5, 0);
					    rootPart.CFrame = old_cframe;
					end;
				else
					resetCamera()
                end;
        ...
        do
            utility.new_connection(run_service.Heartbeat, function(delta_time)
                ...
                local walkspeed_enabled = flags["movement_walkspeed_enabled"];
                local walkspeed_keybind = flags["movement_walkspeed_keybind"];
                local rapidfire_enabled = flags["misc_rapidfire_enabled"];
                local no_slowdown_enabled = flags["misc_no_slowdown_enabled"];
        ...
        -- Fly logic
                ...
                local function StartFly()
                    ...
                    utility.new_connection(run_service.RenderStepped, function()
                        ...
                        if user_input_service:IsKeyDown(Enum.KeyCode.A) then
                            moveDirection = moveDirection - camera.CFrame.RightVector
                        end
                        if user_input_service:IsKeyDown(Enum.KeyCode.D) then
                            moveDirection = moveDirection + camera.CFrame.RightVector
                        end
                        if user_input_service:IsKeyDown(Enum.KeyCode.Space) then
                            moveDirection = moveDirection + Vector3.new(0, 1, 0)
                        end
                        if user_input_service:IsKeyDown(Enum.KeyCode.LeftControl) then
                            moveDirection = moveDirection - Vector3.new(0, 1, 0)
                        end
                        BV.Velocity = moveDirection * cframe_fly_speed
                ...
                -- Handle fly toggle
                if cframe_fly_enabled and cframe_fly_keybind and utility.has_character(local_player) then
                    StartFly()
                else
                    StopFly()
                end
                ...
                if (utility.has_character(local_player)) then
                    local humanoid = local_player.Character:FindFirstChildOfClass("Humanoid");
                    if (humanoid) then
                        if (walkspeed_enabled and walkspeed_keybind) then
                            humanoid.WalkSpeed = flags["movement_walkspeed_amount"];
                            humanoid.JumpPower = flags["movement_jump_power_amount"];
                        else
                ...
                if (rapidfire_enabled and utility.has_character(local_player) and user_input_service:IsMouseButtonPressed(Enum.UserInputType.MouseButton1 or Enum.UserInputType.Touch)) then
                    local gun = dadada.get_gun();
                    if (gun) then
                        dadada.rapid(gun);
                        task.wait(0);
                    end;
                ...
                if (no_slowdown_enabled and utility.has_character(local_player)) then
                    ...
                    if (bodyEffects) then
                        ...
                        if (movement) then
                            ...
                            if (noJumping) then noJumping:Destroy(); print("No Slowdown: Removed NoJumping"); end
                            local reduceWalk = movement:FindFirstChild("ReduceWalk");
                            if (reduceWalk) then reduceWalk:Destroy(); print("No Slowdown: Removed ReduceWalk"); end
                            local noWalkSpeed = movement:FindFirstChild("NoWalkSpeed");
                            if (noWalkSpeed) then noWalkSpeed:Destroy(); print("No Slowdown: Removed NoWalkSpeed"); end
                        end
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">Vhuy</font><font color="rgb(255, 255, 255)">.Ghoul</font>';
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if ((enabled or classics_enabled) and (keybind or classics_keybind) and (not (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer") or (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer"))) then
					local hrp = local_player.Character.HumanoidRootPart and local_player.Character:FindFirstChild("HumanoidRootPart");
					...
					local types = {
						...
						["Destroy Cheaters"] = hrp.CFrame + Vector3.new(11223344556677889900, 1, 1),
						["supercoolboi34 Destroyer"] = locals.should_starhook_destroy and hrp.CFrame + Vector3.new(11223344556677889900, 1, 1) or hrp.CFrame
					};
					...
					run_service.RenderStepped:Wait();
					desync_setback.CFrame = locals.original_position * CFrame.new(0, hrp.Size.Y / 2 + 0.5, 0)
...
if hookmetamethod then
    ...
    mt.__index = newcclosure(function(self, key)
        -- Chỉ hook khi không phải script chính gọi (tránh detection + an toàn hơn)
        if not checkcaller() and self == Mouse and (key == "Hit" or key == "hit" or key == "Target" or key == "target") then

            -- Ưu tiên Rage Target Aim trước (vì rage mạnh hơn, nên để nó override legit)
            if flags["rage_target_aim_enabled"] and locals.target_aim.is_targetting and locals.target_aim.target then
                return locals.target_aim.predicted_position -- hoặc locals.target_aim.predicted_cframe nếu bạn dùng CFrame
            end

            -- Sau đó mới tới Legit Silent Aim
            if flags["legit_silent_enabled"] 
            and not flags["legit_silent_anti_aim_viewer"] 
            and locals.silent_aim.is_targetting 
            and locals.silent_aim.target then
                return locals.silent_aim.predicted_position
            end
        end
...
do
    ...
    ui.tabs["players"] = window:Page({Name = "Players", Icon = "http://www.roblox.com/asset/?id=117259180607823"});
    ...
    do
        local playersLeft = ui.tabs["players"]:Section({Name = "Actions", Side = "Left", Size = 250});
        local playersRight = ui.tabs["players"]:Section({Name = "Player List", Side = "Right", Size = 200});

        local PlayerTarget = nil

        playersLeft:Button({Name = "TP Players", Callback = function()
            if PlayerTarget and PlayerTarget.Character and local_player.Character then
			    local root = local_player.Character and local_player.Character:FindFirstChild("HumanoidRootPart")
			    if root then
			        local pos = PlayerTarget.Character.HumanoidRootPart.Position
        ...
        local function updatePlayerList()
            ...
            for _, plr in ipairs(game.Players:GetPlayers()) do
                if plr ~= local_player then
                    table.insert(playerOptions, string.format("%s (%s)", plr.DisplayName, plr.Name))
                end
            end
            playerList:Refresh(playerOptions)
        end

        playerList = playersRight:List({
            Name = "Select Player",
            ...
            Callback = function(selectedOption)
                PlayerTarget = nil
                for _, plr in ipairs(game.Players:GetPlayers()) do
                    if selectedOption == string.format("%s (%s)", plr.DisplayName, plr.Name) then
                        PlayerTarget = plr
                        break
                    end
                end
        ...
        game.Players.PlayerRemoving:Connect(function(plr)
            updatePlayerList()
            if PlayerTarget == plr then
                PlayerTarget = nil
                Library.Flags["selected_player"] = ""
            end
        end)
    end;
	...
	do
        ...
        local cframe_walkspeed = ui.tabs["movement"]:Section({Name = "WalkSpeed", Side = "Left", Size = 125});
		...
		do
			...
			cframe_fly:Slider({Name = "Speed", Flag = "rage_cframe_fly_amount", Default = 100, Minimum = 1, Maximum = 300, Decimals = 0.01, Ending = "%"});
		end;

    	do
	        local main_toggle = cframe_walkspeed:Toggle({Name = "Enabled", Flag = "movement_walkspeed_enabled"});
	        cframe_walkspeed:Keybind({Flag = "movement_walkspeed_keybind", Name = "Keybind", Default = Enum.KeyCode.T, Mode = "Toggle"});
	        cframe_walkspeed:Slider({Name = "Speed", Flag = "movement_walkspeed_amount", Default = 100, Minimum = 16, Maximum = 500, Decimals = 0.01, Ending = "%"});
            cframe_walkspeed:Slider({Name = "JumpPower", Flag = "movement_jump_power_amount", Default = 55, Minimum = 50, Maximum = 200, Decimals = 1, Ending = ""});
  	    end;
		...
		do
  		  ...
  		  local function applyStats(humanoid)
      		  ...
      		  humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
       		     if flags["movement_walkspeed_enabled"] and flags["movement_walkspeed_keybind"] then
       		         humanoid.WalkSpeed = flags["movement_walkspeed_amount"];
      		      end
    		    end)
          ...
          local function applyNoSlowdown(character)
                if not character then return end
                if not flags["misc_no_slowdown_enabled"] then return end
                local bodyEffects = character:FindFirstChild("BodyEffects")
                if (bodyEffects) then
                    local movement = bodyEffects:FindFirstChild("Movement")
                    if (movement) then
                        local noJumping = movement:FindFirstChild("NoJumping")
                        if (noJumping) then noJumping:Destroy(); print("No Slowdown (CharacterAdded): Removed NoJumping") end
                        local reduceWalk = movement:FindFirstChild("ReduceWalk")
                        if (reduceWalk) then reduceWalk:Destroy(); print("No Slowdown (CharacterAdded): Removed ReduceWalk") end
                        local noWalkSpeed = movement:FindFirstChild("NoWalkSpeed")
                        if (noWalkSpeed) then noWalkSpeed:Destroy(); print("No Slowdown (CharacterAdded): Removed NoWalkSpeed") end
                    end
                    local reload = bodyEffects:FindFirstChild("Reload")
                    if (reload and reload.Value) then
                        reload.Value = false
            ...
            if LocalPlayer.Character then
                ...
                applyNoSlowdown(LocalPlayer.Character)
            end

            LocalPlayer.CharacterAdded:Connect(function(char)
                local humanoid = char:WaitForChild("Humanoid")
                applyStats(humanoid)
                applyNoSlowdown(char)
            end)
		end;
		...
		do
            ...
            misc:Toggle({Name = "RapidFire", Flag = "misc_rapidfire_enabled"});
            misc:Toggle({Name = "Anti Void", Flag = "misc_anti_void_enabled", Callback = function(state)
	            if state then
            ...
            misc:Toggle({Name = "No Slowdown", Flag = "misc_no_slowdown_enabled"});
		end;
	end;
	...
	do --// credits to finobe wtv im way too lazy
		...
		window:Colorpicker({Name = "Menu Accent", Flag = "MenuAccent", Default = Color3.fromRGB(255, 182, 193), Callback = function(state)
			library:ChangeAccent(state);
		end});

        window:Button({Name = "Rejoin", Callback = function()
            local success, error = pcall(function()
                game:GetService("TeleportService"):Teleport(game.PlaceId);
            end);
            if not success then
                print("Rejoin failed:", error);
            end;
        end});
        ...
        local function MakeButton(ButtonKeybind, ButtInstanceName)
                local player = game.Players.LocalPlayer
                local parent = game:GetService("RunService"):IsStudio() and player.PlayerGui or player:WaitForChild("PlayerGui")

                local screenGui = Instance.new("ScreenGui")
                screenGui.Name = ButtInstanceName or "CustomButtonGui"
                screenGui.DisplayOrder = 100
                screenGui.Parent = parent

                local Button = Instance.new("TextButton")
                Button.Parent = screenGui -- Gán button vào ScreenGui
                Button.Name = ButtonKeybind
                Button.Size = UDim2.new(0, 60, 0, 60) -- làm nhỏ gọn
